...
public class LOTRGameRules {
   ...
   private static RuleKey register(String ruleName, Category category, RuleType type) {
      ...
      RuleKey ruleKey = GameRules.func_234903_a_(namespacedName, category, type);
      return ruleKey;
   }

   private static RuleType createBoolean(boolean defaultValue) {
      try {
         Method m = ObfuscationReflectionHelper.findMethod(BooleanValue.class, "func_223568_b", new Class[]{Boolean.TYPE});
   ...
   private static RuleType createInt(int defaultValue) {
      try {
         Method m = ObfuscationReflectionHelper.findMethod(IntegerValue.class, "func_223559_b", new Class[]{Integer.TYPE});
         return (RuleType)m.invoke((Object)null, defaultValue);
      } catch (Exception var2) {
