...
public class LOTRVillageGenSouthron extends LOTRVillageGen {
   ...
   protected LOTRVillageGen.AbstractInstance createVillageInstance(World world, int i, int k, Random random, LocationInfo loc) {
      return new LOTRVillageGenSouthron.Instance(this, world, i, k, random, loc);
   }

   public static class Instance extends LOTRVillageGen.AbstractInstance {
      public LOTRVillageGenSouthron.VillageType villageType;
      ...
      protected void setupVillageProperties(Random random) {
         if (random.nextInt(4) == 0) {
            this.villageType = LOTRVillageGenSouthron.VillageType.FORT;
         } else if (random.nextInt(3) == 0) {
            this.villageType = LOTRVillageGenSouthron.VillageType.TOWN;
         } else {
            this.villageType = LOTRVillageGenSouthron.VillageType.VILLAGE;
         }
      ...
      protected void addVillageStructures(Random random) {
         if (this.villageType == LOTRVillageGenSouthron.VillageType.VILLAGE) {
            this.setupVillage(random);
         } else if (this.villageType == LOTRVillageGenSouthron.VillageType.TOWN) {
            this.setupTown(random);
         } else if (this.villageType == LOTRVillageGenSouthron.VillageType.FORT) {
      ...
      private void setupVillage(Random random) {
         ...
         for(int l = -farmRange; l <= farmRange; ++l) {
            ...
            r = 3;
            if (random.nextInt(3) == 0) {
               this.addStructure(new LOTRWorldGenHayBales(false), farmX, k, r);
            } else {
               this.addStructure(this.getRandomFarm(random), farmX, k, r);
            }
         }
         ...
         for(int l = -houseRange; l <= houseRange; ++l) {
            ...
            r = 0;
            if (Math.abs(i) >= 7) {
               this.addStructure(this.getRandomHouse(random), i, houseZ, r);
            }
         }
      ...
      protected LOTRWorldGenStructureBase2 getRandomHouse(Random random) {
         if (random.nextInt(6) == 0) {
            ...
            return (LOTRWorldGenStructureBase2)(random.nextInt(6) == 0 ? new LOTRWorldGenSouthronStables(false) : new LOTRWorldGenSouthronHouse(false));
         }
      }

      protected LOTRWorldGenStructureBase2 getRandomFarm(Random random) {
         return (LOTRWorldGenStructureBase2)(random.nextBoolean() ? new LOTRWorldGenSouthronFarm(false) : new LOTRWorldGenSouthronPasture(false));
      }
      ...
      private void setupTown(Random random) {
         ...
         int[] var2 = new int[]{-30, 30};
         int i = var2.length;

         int k;
         for(k = 0; k < i; ++k) {
            int i1 = var2[k];
            int[] var6 = new int[]{-30, 30};
      ...
      private void setupFort(Random random) {
         ...
         int[] var2 = new int[]{-25, 25};
         int i = var2.length;

         int k;
         for(k = 0; k < i; ++k) {
            int i1 = var2[k];
            int[] var6 = new int[]{-25, 25};
            int var7 = var6.length;
      ...
      protected LOTRRoadType getPath(Random random, int i, int k) {
         ...
         if (this.villageType == LOTRVillageGenSouthron.VillageType.VILLAGE) {
            ...
            int imn = 45 - random.nextInt(3);
            imx = 50 + random.nextInt(3);
            int kmn = 45 - random.nextInt(3);
            ...
            if (k < 0) {
               imn = 14;
               int imx = 45;
               if (i1 + k1 >= imn + imn && i1 + k1 <= imx + imx) {
                  int d1 = Math.abs(i1 - k1);
                  if (d1 <= (int)(2.5F + (float)random.nextInt(3) * 2.0F)) {
                     return LOTRRoadType.PATH;
                  }
               }
            }
            ...
            if (k > 0) {
               ...
               imn = imn - random.nextInt(3);
               kmn = 14;
               int kmx = 45;
               if (k1 >= kmn && k1 <= kmx && i1 >= imn && i1 <= imx) {
                  return LOTRRoadType.PATH;
               }
            }
         }

         if (this.villageType == LOTRVillageGenSouthron.VillageType.TOWN && i1 <= 72 && k1 <= 42) {
            return LOTRRoadType.HARAD_TOWN;
         } else {
            if (this.villageType == LOTRVillageGenSouthron.VillageType.FORT) {
               if (i1 <= 3 && k >= -45 && k <= -15) {
                  return LOTRRoadType.PATH;
               }
      ...
      public boolean isVillageSpecificSurface(World world, int i, int j, int k) {
         if (this.villageType == LOTRVillageGenSouthron.VillageType.TOWN) {
