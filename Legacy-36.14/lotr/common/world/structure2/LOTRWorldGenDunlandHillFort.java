...
public class LOTRWorldGenDunlandHillFort extends LOTRWorldGenDunlandStructure {
   ...
   public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
      ...
      if (this.restrictions) {
         ...
         for(i2 = -12; i2 <= 12; ++i2) {
            for(k2 = -12; k2 <= 12; ++k2) {
               randomGround = this.getTopBlock(world, i2, k2) - 1;
               if (!this.isSurface(world, i2, randomGround, k2)) {
                  return false;
               }
      ...
      for(i1 = -11; i1 <= 11; ++i1) {
         for(k1 = -11; k1 <= 11; ++k1) {
            ...
            for(randomGround = -1; !this.isOpaque(world, i1, randomGround, k1) && this.getY(randomGround) >= 0; --randomGround) {
               this.setBlockAndMetadata(world, i1, randomGround, k1, Blocks.field_150347_e, 0);
               this.setGrassToDirt(world, i1, randomGround - 1, k1);
            }
         }
      }
      ...
      this.placeChest(world, random, 6, 1, -8, Blocks.field_150486_ae, 3, LOTRChestContents.DUNLENDING_HOUSE);
      this.placeChest(world, random, 5, 1, -8, Blocks.field_150486_ae, 3, LOTRChestContents.DUNLENDING_HOUSE);

      for(i1 = -6; i1 <= -5; ++i1) {
         int j1 = 1;
         int k1 = 8;
         if (random.nextBoolean()) {
            this.placeArmorStand(world, i1, j1, k1, 0, new ItemStack[]{new ItemStack(LOTRMod.helmetDunlending), new ItemStack(LOTRMod.bodyDunlending), new ItemStack(LOTRMod.legsDunlending), new ItemStack(LOTRMod.bootsDunlending)});
         } else {
            this.placeArmorStand(world, i1, j1, k1, 0, new ItemStack[]{new ItemStack(LOTRMod.helmetFur), new ItemStack(LOTRMod.bodyFur), new ItemStack(LOTRMod.legsFur), new ItemStack(LOTRMod.bootsFur)});
         }
      }
      ...
      for(randomGround = 0; randomGround < warriors; ++randomGround) {
         ...
         ((LOTREntityDunlendingWarrior)warrior).spawnRidingHorse = false;
         this.spawnNPCAndSetHome((EntityCreature)warrior, world, 0, 1, 2, 16);
      }
