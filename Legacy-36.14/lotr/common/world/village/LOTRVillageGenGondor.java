...
public class LOTRVillageGenGondor extends LOTRVillageGen {
   ...
   protected LOTRVillageGen.AbstractInstance createVillageInstance(World world, int i, int k, Random random, LocationInfo loc) {
      return new LOTRVillageGenGondor.Instance(this, world, i, k, random, loc);
   }

   public static class Instance extends LOTRVillageGen.AbstractInstance {
      public LOTRVillageGenGondor.VillageType villageType;
      private LOTRWorldGenGondorStructure.GondorFiefdom villageFief;
      ...
      protected void setupVillageProperties(Random random) {
         ...
         if (random.nextInt(4) == 0) {
            this.villageType = LOTRVillageGenGondor.VillageType.FORT;
         } else if (random.nextInt(4) == 0) {
            this.villageType = LOTRVillageGenGondor.VillageType.TOWN;
         } else {
            this.villageType = LOTRVillageGenGondor.VillageType.VILLAGE;
         }

      }

      public boolean isFlat() {
         LOTRVillageGenGondor.VillageType var10001 = this.villageType;
         return this.villageType == LOTRVillageGenGondor.VillageType.TOWN;
      }
      ...
      protected void addVillageStructures(Random random) {
         if (this.villageType == LOTRVillageGenGondor.VillageType.VILLAGE) {
            this.setupVillage(random);
         } else if (this.villageType == LOTRVillageGenGondor.VillageType.TOWN) {
            this.setupTown(random);
         } else if (this.villageType == LOTRVillageGenGondor.VillageType.FORT) {
      ...
      private void setupVillage(Random random) {
         ...
         while(turn < 1.0F) {
            ...
            if (random.nextBoolean()) {
               ...
               this.addStructure(this.getRandomHouse(random), i, k, farmZ);
            } else if (random.nextInt(3) != 0) {
               l = 65;
               i = Math.round((float)l * cos);
               k = Math.round((float)l * sin);
               this.addStructure(new LOTRWorldGenHayBales(false), i, k, farmZ);
            }
         }

         int signPos = Math.round(50.0F * MathHelper.func_76134_b((float)Math.toRadians(45.0D)));
         int signDisp = Math.round(7.0F * MathHelper.func_76134_b((float)Math.toRadians(45.0D)));
      ...
      private LOTRWorldGenStructureBase2 getRandomFarm(Random random) {
         if (random.nextBoolean()) {
            return (LOTRWorldGenStructureBase2)(random.nextBoolean() ? new LOTRWorldGenGondorVillageFarm.Animals(false) : new LOTRWorldGenGondorVillageFarm.Crops(false));
         } else {
      ...
      private void setupTown(Random random) {
         ...
         int flowerX = 12;
         int flowerZ = 18;
         int[] var26 = new int[]{-flowerX, flowerX};
         int var28 = var26.length;

         for(k1 = 0; k1 < var28; ++k1) {
            houseZ = var26[k1];
            this.addStructure(new LOTRWorldGenGondorTownGarden(false), houseZ, flowerZ, 0, true);
            this.addStructure(new LOTRWorldGenGondorTownGarden(false), houseZ, -flowerZ, 2, true);
            this.addStructure(new LOTRWorldGenGondorTownGarden(false), -flowerZ, houseZ, 1, true);
            this.addStructure(new LOTRWorldGenGondorTownGarden(false), flowerZ, houseZ, 3, true);
         }
         ...
         for(houseZ = 0; houseZ < k1; ++houseZ) {
            k1 = var7[houseZ];
            houseZ = k1 * 6;
            this.addStructure(new LOTRWorldGenGondorLampPost(false), houseZ, lampZ, 0, true);
            this.addStructure(new LOTRWorldGenGondorLampPost(false), houseZ, -lampZ, 2, true);
            if (k1 != -1) {
               this.addStructure(new LOTRWorldGenGondorLampPost(false), -lampZ, houseZ, 1, true);
            }

            this.addStructure(new LOTRWorldGenGondorLampPost(false), lampZ, houseZ, 3, true);
         }
         ...
         int[] var30 = new int[]{-43, -51};
         houseZ = var30.length;

         for(k1 = 0; k1 < houseZ; ++k1) {
            houseZ = var30[k1];
            this.addStructure(new LOTRWorldGenGondorTownBench(false), houseZ, -9, 2, true);
            this.addStructure(new LOTRWorldGenGondorTownBench(false), houseZ, -3, 0, true);
         }
         ...
         int wellX = 22;
         int wellZ = 31;
         int[] var33 = new int[]{-wellX, wellX};
         houseZ = var33.length;

         int k1;
         for(int var12 = 0; var12 < houseZ; ++var12) {
            k1 = var33[var12];
            this.addStructure(new LOTRWorldGenGondorWell(false), k1, -wellZ, 2, true);
            this.addStructure(new LOTRWorldGenGondorWell(false), k1, wellZ, 0, true);
            this.addStructure(new LOTRWorldGenGondorWell(false), -wellZ, k1, 1, true);
            this.addStructure(new LOTRWorldGenGondorWell(false), wellZ, k1, 3, true);
         }
         ...
         int towerX = 78;
         int towerZ = 74;
         int[] var44 = new int[]{-towerX, towerX};
         k1 = var44.length;
         ...
         for(l = 0; l < k1; ++l) {
            wallX = var44[l];
            this.addStructure(this.getVillageWatchtower(), wallX, -towerZ, 2, true);
            this.addStructure(this.getVillageWatchtower(), wallX, towerZ, 0, true);
         }
      ...
      private void setupFortVillage(Random random) {
         ...
         int[] var2 = new int[]{-20, 20};
         int var3 = var2.length;

         for(int var4 = 0; var4 < var3; ++var4) {
            int i1 = var2[var4];
            int[] var6 = new int[]{-20, 20};
      ...
      private LOTRWorldGenStructureBase2 getVillageFortress() {
         ...
         } else if (this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.LOSSARNACH) {
            return new LOTRWorldGenLossarnachFortress(false);
         } else if (this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.LEBENNIN) {
            return new LOTRWorldGenLebenninFortress(false);
         } else if (this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.PELARGIR) {
            return new LOTRWorldGenPelargirFortress(false);
         } else if (this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.PINNATH_GELIN) {
            return new LOTRWorldGenPinnathGelinFortress(false);
         } else if (this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.BLACKROOT_VALE) {
            return new LOTRWorldGenBlackrootFortress(false);
         } else if (this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.LAMEDON) {
            return new LOTRWorldGenLamedonFortress(false);
         } else {
            return this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.DOL_AMROTH ? new LOTRWorldGenDolAmrothStables(false) : null;
         }
      }
      ...
      private LOTRWorldGenStructureBase2 getVillageWatchtower() {
         if (this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.GONDOR) {
            ...
            return this.villageFief == LOTRWorldGenGondorStructure.GondorFiefdom.DOL_AMROTH ? new LOTRWorldGenDolAmrothWatchtower(false) : null;
         }
      }
      ...
      protected LOTRRoadType getPath(Random random, int i, int k) {
         ...
         if (this.villageType == LOTRVillageGenGondor.VillageType.TOWN && i1 <= 80 && k1 <= 80) {
            return LOTRRoadType.COBBLESTONE;
         } else {
            if (this.villageType == LOTRVillageGenGondor.VillageType.FORT) {
      ...
      public boolean isVillageSpecificSurface(World world, int i, int j, int k) {
         ...
         return this.villageType == LOTRVillageGenGondor.VillageType.TOWN && block == Blocks.field_150347_e;
      }
   }
