...
public class LOTRWorldGenBlackUrukFort extends LOTRWorldGenMordorStructure {
   ...
   public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
      ...
      if (this.restrictions) {
         ...
         for(l = -18; l <= 26; ++l) {
            for(k1 = -18; k1 <= 20; ++k1) {
               i1 = this.getTopBlock(world, l, k1) - 1;
               if (!this.isSurface(world, l, i1, k1)) {
                  return false;
               }
      ...
      for(k1 = -17; k1 <= 25; ++k1) {
         for(maxHeight = -18; maxHeight <= 19; ++maxHeight) {
            for(l = 1; l <= 16; ++l) {
               this.setAir(world, k1, l, maxHeight);
            }
         }
      }
      ...
      for(k1 = 4; k1 <= 5; ++k1) {
         ...
         for(k1 = 0; k1 < l; ++k1) {
            i1 = var12[k1];
            this.setBlockAndMetadata(world, i1, k1, 17, this.bedBlock, 0);
            this.setBlockAndMetadata(world, i1, k1, 18, this.bedBlock, 8);
         }

         var12 = new int[]{6, 8, 10};
         l = var12.length;
      ...
      for(l = 0; l < uruks; ++l) {
         ...
         ((LOTREntityBlackUruk)uruk).spawnRidingHorse = false;
         this.spawnNPCAndSetHome((EntityCreature)uruk, world, 0, 1, 0, 32);
      }
