package lotr.common.world.biome.variant;

import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;

public class LOTRBiomeVariantList {
   private float totalWeight;
   private List variantList = new ArrayList();

   public void add(LOTRBiomeVariant v, float f) {
      this.variantList.add(new LOTRBiomeVariantList.VariantBucket(v, this.totalWeight, this.totalWeight + f));
      this.totalWeight += f;
   }

   public void clear() {
      this.totalWeight = 0.0F;
      this.variantList.clear();
   }

   public LOTRBiomeVariant get(float index) {
      if (index < 0.0F) {
         index = 0.0F;
      }

      if (index >= 1.0F) {
         index = 0.9999F;
      }

      float f = index * this.totalWeight;
      Iterator var3 = this.variantList.iterator();

      LOTRBiomeVariantList.VariantBucket bucket;
      do {
         if (!var3.hasNext()) {
            return null;
         }

         bucket = (LOTRBiomeVariantList.VariantBucket)var3.next();
      } while(f < bucket.min || f >= bucket.max);

      return bucket.variant;
   }

   public boolean isEmpty() {
      return this.totalWeight == 0.0F;
   }

   private static class VariantBucket {
      public final LOTRBiomeVariant variant;
      public final float min;
      public final float max;

      private VariantBucket(LOTRBiomeVariant v, float f0, float f1) {
         this.variant = v;
         this.min = f0;
         this.max = f1;
      }

      // $FF: synthetic method
      VariantBucket(LOTRBiomeVariant x0, float x1, float x2, Object x3) {
         this(x0, x1, x2);
      }
   }
}
