...
public class LOTRWorldGenDaleVillage extends LOTRWorldGenStructureBase2 {
   ...
   public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
      ...
      if (!tower.generateWithSetRotation(world, random, this.getX(i1, k1), this.getY(j1), this.getZ(i1, k1), (this.getRotationMode() + r) % 4)) {
         ...
         int smithies = MathHelper.func_76136_a(random, 0, 1);

         int bakeries;
         for(bakeries = 0; bakeries < smithies; ++bakeries) {
         ...
         bakeries = MathHelper.func_76136_a(random, 0, 1);

         int houses;
         for(houses = 0; houses < bakeries; ++houses) {
         ...
         houses = MathHelper.func_76136_a(random, 2, 5);
   ...
   private boolean tryGenerateHouse(World world, Random random, LOTRWorldGenStructureBase2 structure) {
      ...
      for(int l = 0; l < attempts; ++l) {
         ...
         if (r == 0) {
            i1 = MathHelper.func_76136_a(random, -16, 16);
            k1 = MathHelper.func_76136_a(random, 7, 10);
         } else if (r == 1) {
            k1 = MathHelper.func_76136_a(random, -16, 16);
            i1 = -MathHelper.func_76136_a(random, 7, 10);
         } else if (r == 2) {
            i1 = MathHelper.func_76136_a(random, -16, 16);
            k1 = -MathHelper.func_76136_a(random, 7, 10);
         } else if (r == 3) {
