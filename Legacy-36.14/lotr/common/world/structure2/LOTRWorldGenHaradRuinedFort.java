...
public class LOTRWorldGenHaradRuinedFort extends LOTRWorldGenStructureBase2 {
   ...
   public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
      ...
      for(i1 = -7; i1 <= 12; ++i1) {
         for(k1 = -3; k1 <= 4; ++k1) {
            for(j1 = -2; !this.isOpaque(world, i1, j1, k1) && this.getY(j1) >= 0; --j1) {
      ...
      while(chestCoords.size() > k1) {
         chestCoords.remove(random.nextInt(chestCoords.size()));
      }

      Iterator var12 = chestCoords.iterator();

      while(var12.hasNext()) {
         int[] coords = (int[])var12.next();
         this.placeChest(world, random, coords[0], coords[1], coords[2], LOTRMod.chestBasket, MathHelper.func_76136_a(random, 2, 4), LOTRChestContents.NEAR_HARAD_PYRAMID);
      }
