...
public abstract class LOTRWorldGenElvenForge extends LOTRWorldGenStructureBase2 {
   ...
   public LOTRWorldGenElvenForge(boolean flag) {
      super(flag);
      this.chestBlock = Blocks.field_150486_ae;
   }
   ...
   public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
      ...
      for(i1 = -2; i1 <= 2; ++i1) {
         this.layFoundation(world, -5, i1, random);
         this.layFoundation(world, 5, i1, random);
      }
      ...
      for(i1 = 0; i1 <= 3; ++i1) {
         ...
         for(k2 = -k1; k2 <= k1; ++k2) {
            this.placeRoofStairs(world, k2, i2, -k1, 2, random);
            this.placeRoofStairs(world, k2, i2, k1, 3, random);
            this.placeRoofStairs(world, k2, i2 - 1, -k1, 7, random);
            this.placeRoofStairs(world, k2, i2 - 1, k1, 6, random);
         }

         for(k2 = -k1 + 1; k2 <= k1 - 1; ++k2) {
            this.placeRoofStairs(world, -k1, i2, k2, 1, random);
            this.placeRoofStairs(world, k1, i2, k2, 0, random);
            this.placeRoofStairs(world, -k1, i2 - 1, k2, 4, random);
            this.placeRoofStairs(world, k1, i2 - 1, k2, 5, random);
         }
         ...
         if (i1 < 3) {
            ...
            for(j1 = -k2; j1 <= k2; ++j1) {
               this.placeRoofStairs(world, j1, i2, -k1 - 1, 2, random);
               this.placeRoofStairs(world, j1, i2, k1 + 1, 3, random);
               this.placeRoof(world, j1, i2, -k1, random);
               this.placeRoof(world, j1, i2, k1, random);
            }

            for(j1 = -k2; j1 <= k2; ++j1) {
               this.placeRoofStairs(world, -k1 - 1, i2, j1, 1, random);
               this.placeRoofStairs(world, k1 + 1, i2, j1, 0, random);
               this.placeRoof(world, -k1, i2, j1, random);
               this.placeRoof(world, k1, i2, j1, random);
            }
            ...
            if (k2 > 0) {
               for(j1 = 0; j1 <= 1; ++j1) {
                  for(int l2 = 0; l2 <= 1; ++l2) {
                     ...
                     this.placeRoofStairs(world, -k2, i2, l3 * (k1 + j1), 1, random);
                     this.placeRoofStairs(world, k2, i2, l3 * (k1 + j1), 0, random);
                     this.placeRoofStairs(world, l3 * (k1 + j1), i2, -k2, 2, random);
                     this.placeRoofStairs(world, l3 * (k1 + j1), i2, k2, 3, random);
                  }
               }
            }
         }
      ...
      if (!this.ruined) {
         for(i1 = -4; i1 <= 4; i1 += 8) {
            this.setBlockAndMetadata(world, i1, 2, -1, Blocks.field_150467_bQ, 0);
      ...
      if (!this.ruined) {
         ...
         this.setBlockAndMetadata(world, 0, 2, 4, Blocks.field_150462_ai, 0);
         this.placeChest(world, random, 1, 2, 4, this.chestBlock, 2, LOTRChestContents.ELVEN_FORGE);
      }
      ...
      if (!this.ruined) {
         this.setBlockAndMetadata(world, 0, 2, 0, Blocks.field_150353_l, 0);
      }
      ...
      LOTREntityElf elf = this.getElf(world);
      if (elf != null) {
   ...
   private void layFoundation(World world, int i, int k, Random random) {
      int j;
      for(j = 0; !this.isOpaque(world, i, j, k) && this.getY(j) >= 0; --j) {
