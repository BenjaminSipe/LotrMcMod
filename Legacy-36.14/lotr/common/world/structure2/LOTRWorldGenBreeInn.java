...
public class LOTRWorldGenBreeInn extends LOTRWorldGenBreeStructure {
   ...
   public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
      ...
      this.setBlockAndMetadata(world, -6, 2, -3, getRandomPieBlock(random), 0);
      ...
      this.placeSign(world, -2, 4, -7, Blocks.field_150444_as, 2, new String[]{"", innName[0], innName[1], ""});
      this.placeSign(world, -1, 4, -6, Blocks.field_150444_as, 4, new String[]{"", innName[0], innName[1], ""});
      this.placeSign(world, -3, 4, -6, Blocks.field_150444_as, 5, new String[]{"", innName[0], innName[1], ""});
      Object innkeeper;
      if (this.hasPresets) {
      ...
      if (this.hasPresets) {
         ((LOTREntityNPC)innkeeper).familyInfo.setMale(this.presetIsMaleKeeper);
         ((LOTREntityNPC)innkeeper).familyInfo.setName(this.presetInnkeeperName);
      }

      ((LOTREntityNPC)innkeeper).setSpecificLocationName(innNameNPC);
      this.spawnNPCAndSetHome((EntityCreature)innkeeper, world, -5, 1, 0, 4);
      String[] innkeeperNameParts = ((LOTREntityNPC)innkeeper).getNPCName().split(" ");
      if (innkeeperNameParts.length < 2) {
         innkeeperNameParts = new String[]{innkeeperNameParts[0], ""};
      }

      this.placeSign(world, -2, 3, -5, Blocks.field_150444_as, 2, new String[]{"", "by " + innkeeperNameParts[0], innkeeperNameParts[1], ""});
      int men = 8 + random.nextInt(6);

      for(int l = 0; l < men; ++l) {
         LOTREntityNPC breelander = random.nextInt(3) == 0 ? new LOTREntityBreeHobbit(world) : new LOTREntityBreeMan(world);
         if (random.nextInt(10) == 0) {
            breelander = new LOTREntityRuffianSpy(world);
         }

         this.spawnNPCAndSetHome((EntityCreature)breelander, world, -2, 1, 0, 16);
      }
