...
public class LOTRWorldGenRohanFortWall extends LOTRWorldGenRohanStructure {
   ...
   public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
      ...
      for(int i1 = this.xMin; i1 <= this.xMax; ++i1) {
         ...
         int h;
         for(h = 1; (h >= 0 || !this.isOpaque(world, i1, h, k1)) && this.getY(h) >= 0; --h) {
            this.setBlockAndMetadata(world, i1, h, k1, this.rockSlabDoubleBlock, this.rockSlabDoubleMeta);
            this.setGrassToDirt(world, i1, h - 1, k1);
         }

         for(h = 2; h <= 2; ++h) {
            this.setBlockAndMetadata(world, i1, h, k1, this.brickBlock, this.brickMeta);
         }

         h = 5 + random.nextInt(2);

         for(int j1 = 3; j1 <= h; ++j1) {
